...
describe("signin controller", () => {
    ...
    it("should sign in an admin user successfully with valid credentials", async () => {
      ...
      expect(res.status.calledOnceWith(200)).to.be.true; // Ensure that status is called with 200
      expect(res.json.calledOnce).to.be.true; // Ensure that json is called once
      expect(res.cookie.calledOnceWith("access_token", sinon.match.string, { httpOnly: false, maxAge: 12096000000, sameSite: 'none', secure: false })).to.be.true; // Ensure that cookie is called with the correct parameters
    });
